  ...
  if (currentTab === 'sklad_gp') {
      try {
          if (!isEditMode) {
              ...
              let payload = { "ID Детайл": det, "Операция": op, "Количество": qty, "Статус": "Изработено", "Оператор": "СИСТЕМА (Ръчно добавен)", "Дата": new Date().toISOString() };
              ...
              if (diff !== 0) {
                  let payload = { "ID Детайл": det, "Операция": op, "Количество": diff, "Статус": "Изработено", "Оператор": "СИСТЕМА (Корекция наличност)", "Дата": new Date().toISOString() };
                  const { error } = await client.from('otcheti').insert([payload]); 
                  if (error) throw error; 
              }
